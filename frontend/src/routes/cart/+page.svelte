<script lang="ts">
  ...
  // --- LOGIC UPDATE QUANTITY (BARU) ---

  // 1. Update via Input Text (Manual Ketik)
  function updateQty(id: string, value: any) {
    let newQty = parseInt(value);

    // Validasi: Jika bukan angka atau < 1, paksa jadi 1
  ...
  function decrementQty(item: any) {
    ...
    if (currentQty > 1) {
      // Jika lebih dari 1, kurangi
      cartStore.setQty(item.id, currentQty - 1);
    } else {
      // Jika sisa 1, hapus item dari cart
  ...
  <div class="w-full max-w-5xl rounded-2xl bg-white shadow-lg p-8">
    ...
    {:else}
          ...
          <div class="flex items-center justify-between py-5">
            ...
            <div class="flex items-center gap-3">
              <button
                ...
                class="rounded-lg bg-zinc-200 px-3 py-1 hover:bg-zinc-300 transition-colors"
              >
                −
              </button>
              ...
              <input
                type="number"
                min="1"
                class="w-16 rounded-lg border border-zinc-300 p-1.5 text-center text-sm focus:border-emerald-500 focus:ring-emerald-500 focus:outline-none"
                value={item.qty}
                on:input={(e) => {
                  const target = e.target as HTMLInputElement;
      ...
      <div class="flex items-center justify-between mb-8">
        ...
        <p class="text-2xl font-bold text-emerald-600">Rp {($grandTotalStore ?? 0).toLocaleString()}</p>
      </div>
